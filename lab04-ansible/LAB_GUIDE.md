# lab04-ansible — Ansible による Leaf-Spine OSPF 自動設定
...
cd ~/git/container_lab/lab04-ansible/ansible
...
```bash
cd ~/git/container_lab/lab04-ansible
...
```bash
docker exec -it clab-lab04-ansible-ceos1 Cli
```
...
spine:
  hosts:
    ceos1:
      ansible_host: clab-lab04-ansible-ceos1   # /etc/hosts に自動登録される
```
...
docker exec clab-lab04-ansible-ceos1 /usr/bin/Cli -c "show ip ospf neighbor"

# Host1（ceos7）の OSPF 隣接：Leaf1（ceos3）のみ Full になっていること
docker exec clab-lab04-ansible-ceos7 /usr/bin/Cli -c "show ip ospf neighbor"
```
...
docker exec clab-lab04-ansible-ceos7 /usr/bin/Cli -c "show ip route ospf"
```
...
docker exec clab-lab04-ansible-ceos7 /usr/bin/Cli -p 15 -c "ping 8.8.8.8 source 7.7.7.7"
```
...
| `Connection refused` エラー | `docker exec clab-lab04-ansible-ceos1 /usr/bin/Cli -c "show management api http-commands"` | management api http-commands が無効 |
