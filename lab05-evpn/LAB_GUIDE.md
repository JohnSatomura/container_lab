...
```
                 ...
                 └──┬──┬──┬──┬┘     └──┬──┬──┬──┬┘
    10.1.0.0/30  ───┘  │  │  └───────  │  │  │  └─── 10.2.0.12/30
    10.1.0.4/30  ──────┘  │   10.2.0.0/30  │  │
    10.1.0.8/30  ─────────┘   10.2.0.4/30  │  │
    10.1.0.12/30 ────────── 10.2.0.8/30    │  │
                                10.2.0.12/30│  │
         │           │           │          │  │
    ┌────┴────┐  ┌───┴────┐  ┌──┴─────┐  ┌─┴──────┐
    ...
    │Lo0:3.3.3│  │Lo0:4.4.4│  │Lo0:5.5.5│  │Lo0:6.6.6│
...
**アンダーレイ（eBGP, IPv4 unicast）**:
- 各 Leaf → Spine1/Spine2（P2P リンク IP で直接 peer）
- Loopback /32 を広告して互いの VTEP IP を到達可能にする

**オーバーレイ（eBGP EVPN, ebgp-multihop 3）**:
- 各 Leaf → Spine1/Spine2（Loopback0 同士で peer）
- Spine が `next-hop-unchanged` を設定し VTEP IP を保持して転送
- VLAN 10 → VNI 10010、RT: 65001:10010（全 Leaf 共通）
