...
```
        ┌──────────────────────────────────────────────────────────┐
        │                     【 Spine 層 】                        │
        │   Spine1 (ceos1)  AS65000  Loopback0: 1.1.1.1/32        │
        │   Spine2 (ceos2)  AS65000  Loopback0: 2.2.2.2/32        │
        │                                                          │
        │   各 Spine は全 Leaf と eBGP peer（合計 8 P2P リンク）   │
        │   Loopback 間 EVPN セッション（ebgp-multihop 3）        │
        └──────┬──────────────┬──────────────┬──────────────┬──────┘
               │              │              │              │
        ┌──────┴──────┐ ┌─────┴──────┐ ┌────┴───────┐ ┌────┴───────┐
        │   Leaf1     │ │   Leaf2    │ │   Leaf3    │ │   Leaf4    │
        │   ceos3     │ │   ceos4    │ │   ceos5    │ │   ceos6    │
        │  AS65001    │ │  AS65002   │ │  AS65003   │ │  AS65004   │
        │  3.3.3.3/32 │ │  4.4.4.4/32│ │  5.5.5.5/32│ │  6.6.6.6/32│
        │  VNI:10010  │ │  VNI:10010 │ │  VNI:10010 │ │  VNI:10010 │
        └──────┬──────┘ └────────────┘ └────────────┘ └──────┬─────┘
               │ Et3                                    Et3   │
           access vlan10                           access vlan10
        ┌──────┴──────┐                             ┌─────────┴───┐
        │   Host1     │◄──────── VXLAN VNI10010 ───►│    Host2    │
        │   ceos7     │          L2 ストレッチ        │    ceos8    │
        │ 192.168.10.1│       同一セグメント扱い      │ 192.168.10.2│
        │    /24      │                              │     /24     │
        └─────────────┘                             └─────────────┘
```
...
### 図2: アンダーレイ接続詳細（eBGP P2P リンク）

```
Spine1 (ceos1, AS65000, Lo:1.1.1.1)
  ├─ Et1: 10.1.0.1/30 ──────── 10.1.0.2/30 :Et1  Leaf1 (ceos3, AS65001, Lo:3.3.3.3)
  ├─ Et2: 10.1.0.5/30 ──────── 10.1.0.6/30 :Et1  Leaf2 (ceos4, AS65002, Lo:4.4.4.4)
  ├─ Et3: 10.1.0.9/30 ──────── 10.1.0.10/30 :Et1 Leaf3 (ceos5, AS65003, Lo:5.5.5.5)
  └─ Et4: 10.1.0.13/30 ─────── 10.1.0.14/30 :Et1 Leaf4 (ceos6, AS65004, Lo:6.6.6.6)

Spine2 (ceos2, AS65000, Lo:2.2.2.2)
  ├─ Et1: 10.2.0.1/30 ──────── 10.2.0.2/30 :Et2  Leaf1 (ceos3)
  ├─ Et2: 10.2.0.5/30 ──────── 10.2.0.6/30 :Et2  Leaf2 (ceos4)
  ├─ Et3: 10.2.0.9/30 ──────── 10.2.0.10/30 :Et2 Leaf3 (ceos5)
  └─ Et4: 10.2.0.13/30 ─────── 10.2.0.14/30 :Et2 Leaf4 (ceos6)

Leaf1 (ceos3)
  └─ Et3: (L2 access vlan10) ── Et1: (access vlan10) Host1 (ceos7, Vlan10: 192.168.10.1/24)

Leaf4 (ceos6)
  └─ Et3: (L2 access vlan10) ── Et1: (access vlan10) Host2 (ceos8, Vlan10: 192.168.10.2/24)
```
...
```
Leaf1 (3.3.3.3) ─── ebgp-multihop 3 ──► Spine1 (1.1.1.1)  ─── ebgp-multihop 3 ──► Leaf2 (4.4.4.4)
Leaf1 (3.3.3.3) ─── ebgp-multihop 3 ──► Spine2 (2.2.2.2)  ─── ebgp-multihop 3 ──► Leaf3 (5.5.5.5)
                                                            ─── ebgp-multihop 3 ──► Leaf4 (6.6.6.6)
※ Leaf2〜Leaf4 も同様に両 Spine へセッションを張る

Spine の役割：
  ・受け取った EVPN ルートを全 Leaf に転送
  ・next-hop-unchanged で VTEP IP（Loopback）を書き換えずに保持
  ・VLAN10 / VNI10010 / Route-Target 65001:10010 を全 Leaf が共有
```
...
|-------|-----|-----------|-----------|-----------|-----------|-----------|
| ceos1 (Spine1) | 65000 | 1.1.1.1/32 | 10.1.0.1/30 → Leaf1 | 10.1.0.5/30 → Leaf2 | 10.1.0.9/30 → Leaf3 | 10.1.0.13/30 → Leaf4 |
| ceos2 (Spine2) | 65000 | 2.2.2.2/32 | 10.2.0.1/30 → Leaf1 | 10.2.0.5/30 → Leaf2 | 10.2.0.9/30 → Leaf3 | 10.2.0.13/30 → Leaf4 |

### Leaf

| ノード | AS | Loopback0 | Ethernet1 (Spine1) | Ethernet2 (Spine2) | Ethernet3 |
|-------|-----|-----------|--------------------|--------------------|-----------|
| ceos3 (Leaf1) | 65001 | 3.3.3.3/32 | 10.1.0.2/30 | 10.2.0.2/30 | access vlan10 → Host1 |
| ceos4 (Leaf2) | 65002 | 4.4.4.4/32 | 10.1.0.6/30 | 10.2.0.6/30 | — |
| ceos5 (Leaf3) | 65003 | 5.5.5.5/32 | 10.1.0.10/30 | 10.2.0.10/30 | — |
| ceos6 (Leaf4) | 65004 | 6.6.6.6/32 | 10.1.0.14/30 | 10.2.0.14/30 | access vlan10 → Host2 |

### Host / VXLAN

| ノード | 役割 | Vlan10 SVI | 接続先 |
|-------|------|-----------|-------|
| ceos7 (Host1) | ホスト | 192.168.10.1/24 | Leaf1(ceos3) Et3 |
| ceos8 (Host2) | ホスト | 192.168.10.2/24 | Leaf4(ceos6) Et3 |
...
│   ├── ceos1.cfg       # Spine1: Loopback + P2P リンク IP
│   ├── ceos2.cfg       # Spine2: Loopback + P2P リンク IP
│   ├── ceos3.cfg       # Leaf1:  L3 uplink + access vlan10(Et3)
│   ├── ceos4.cfg       # Leaf2:  L3 uplink + vlan 10 定義
│   ├── ceos5.cfg       # Leaf3:  L3 uplink + vlan 10 定義
│   ├── ceos6.cfg       # Leaf4:  L3 uplink + access vlan10(Et3)
│   ├── ceos7.cfg       # Host1:  Vlan10 SVI 192.168.10.1/24
│   └── ceos8.cfg       # Host2:  Vlan10 SVI 192.168.10.2/24
└── configs-full/       # フルコンフィグモード（BGP EVPN + VXLAN 全設定済み）
    ├── ceos1.cfg       # Spine1: eBGP underlay + EVPN (next-hop-unchanged)
    ├── ceos2.cfg       # Spine2: eBGP underlay + EVPN (next-hop-unchanged)
    ├── ceos3.cfg       # Leaf1:  eBGP + EVPN + Vxlan1
    ├── ceos4.cfg       # Leaf2:  eBGP + EVPN + Vxlan1
    ├── ceos5.cfg       # Leaf3:  eBGP + EVPN + Vxlan1
    ├── ceos6.cfg       # Leaf4:  eBGP + EVPN + Vxlan1
    ├── ceos7.cfg       # Host1:  configs-init と同一
    └── ceos8.cfg       # Host2:  configs-init と同一
```
...
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show bgp summary"

# Spine1 の BGP セッション（全 Leaf と Estab になること）
docker exec clab-lab05-evpn-ceos1 /usr/bin/Cli -c "show bgp summary"
```
...
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show ip route bgp"

# Leaf1 → Leaf4（VTEP IP = 6.6.6.6）への疎通確認
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -p 15 -c "ping 6.6.6.6 source 3.3.3.3"
```
...
docker exec clab-lab05-evpn-ceos1 /usr/bin/Cli -c "show bgp evpn summary"

# Leaf1 の EVPN セッション（Spine1・Spine2 と Estab になること）
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show bgp evpn summary"
```
...
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show vxlan vtep"
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show vxlan vni"

# Spine1 が保持する全 EVPN ルート
docker exec clab-lab05-evpn-ceos1 /usr/bin/Cli -c "show bgp evpn"
```
...
docker exec clab-lab05-evpn-ceos7 /usr/bin/Cli -p 15 -c "ping 192.168.10.2 source 192.168.10.1 repeat 5"
```
...
```bash
# ping 後に MAC テーブルを確認
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show mac address-table"
docker exec clab-lab05-evpn-ceos6 /usr/bin/Cli -c "show mac address-table"

# EVPN MAC-IP ルートの確認
docker exec clab-lab05-evpn-ceos3 /usr/bin/Cli -c "show bgp evpn route-type mac-ip"
docker exec clab-lab05-evpn-ceos1 /usr/bin/Cli -c "show bgp evpn route-type mac-ip"
```
...
```bash
docker exec -it clab-lab05-evpn-ceos3 Cli
```
